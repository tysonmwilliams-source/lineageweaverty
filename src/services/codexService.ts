...
export interface NewCodexEntry {
  ...
  /**
   * A copy of the person's vital fields, written by `GenealogyContext` when it
   * auto-creates a personage entry.
   *
   * **Nothing reads it.** `genealogyData` appears exactly once in the codebase —
   * the write — so every auto-created person entry carries a duplicate of data
   * that already lives on the person record, and syncs it to Firestore. It is
   * declared rather than removed because deleting a field that reaches the
   * cloud is a behaviour change, and this is a conversion. Recorded as a
   * finding.
   */
  genealogyData?: Record<string, unknown>;
  /**
   * Marks an entry the app created rather than the user.
   *
   * Read by nothing, exactly like `genealogyData` above — the two are written
   * together by the same call and neither has ever been consulted. Declared for
   * the same reason: removing a field that reaches Firestore is a behaviour
   * change and this is a conversion.
   */
